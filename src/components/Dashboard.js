...
      try {
        const q = query(collection(db, 'firstVisit'), orderBy('timestamp', 'desc'), limit(1));
        const querySnapshot = await getDocs(q);

        if (!querySnapshot.empty) {
          const latestData = querySnapshot.docs[0].data();
          const count = latestData.count || 0;
          const totalGarantia = latestData.totalGarantia || 0;

          if (totalGarantia > 0) {
            const notFirstVisit = totalGarantia - count;
            setFirstVisitPieData([
              { name: 'Reparo 1ª Visita', value: count },
              { name: 'Reparo em Outra Visita', value: notFirstVisit }
            ]);
          } else {
            setFirstVisitPieData([]);
          }
...
  ];
  
  // Novo esquema de cores para o gráfico de pizza
 // const PIE_COLORS = ['#32a852', '#d9534f']; // Verde para "Valor Pago", Vermelho para "Faltando para a Meta"
...
        <div className="kpi-chart-container">
          <h3 style={{ marginBottom: '5px' }}>Progresso da Meta de Orçamento CI VD</h3>
          <div style={{ width: '100%', height: 300 }}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
...
        {/* NOVO GRAFICO DE PIZZA DE 1ST VISIT */}
        <div className="kpi-chart-container">
          <h3 style={{ marginBottom: '5px' }}>1ST VISIT % (Último Registro)</h3>
          <div style={{ width: '100%', height: 300 }}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
